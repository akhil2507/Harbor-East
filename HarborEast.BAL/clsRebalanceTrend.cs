...
{
    public class clsRebalanceTrend
    {
        ...
        bool isTREBHistorical = false;
        bool isSplitOrDiv = false;
        #endregion

        #region Constructors

        public clsRebalanceTrend()
        {
        ...
        /// <summary>
        /// This method is used for calculating the values for Rebalance Trend operation.
        /// </summary>
        /// <param name="intSelectedIndex"></param>
        /// <param name="strSelectedValue"></param>
        /// <param name="isChecked"></param>
        /// <returns></returns>
        public DataTable PerformRebalanceTrend(int intSelectedIndex, string strSelectedValue, bool isChecked)
        {
            ...
            try
            {
                ...
                if (intSelectedIndex != -1)
                {
                    ...
                    //Changed code for schedular timing
                    ...
                    isTREBHistorical = CommonUtility.IsHistoricalProcessing();
                        ...
                        if (intArraySize != -1)
                        {
                            ...
                            foreach (var objSet in lstAssetAllocation)
                            {
                                ...
                                for (int intI = 213; intI < objSet.MPrice.Count(); intI++)
                                {
                                    ...
                                    if (intI >= 217)
                                    {
                                        objSet.MSlope[intI] = CommonUtility.Slope(objSet.MSMA.Skip(intSlopeSkipCount).Take(5).Select(x => (double)x).ToArray(), objSet.MDate.Skip(intSlopeSkipCount).Take(5).ToArray()) /objSet.MSMA[intI];
                                        intSlopeSkipCount++;
                                    }
                                }
                            }
                            ...
                            if (isTREBHistorical || isSplitOrDiv)
                            {
                                for (int i = 217; i < lstAssetAllocation[0].MDate.Count(); i++)
                                {
                                    ...
                                    {
                                        objHEEntities.PortfolioValueSet.First(x => x.PortfolioSet.Id.Equals(intID) && x.Date.Equals(dtFillDate)).TREB = (float)decBHPortfolioValue[i];
                                    }
                                    else
                                    {
                                        PortfolioValueSet objPortfolioValueSet = new PortfolioValueSet();
                                        objPortfolioValueSet.Date = dtFillDate;
                                        objPortfolioValueSet.TREB = (float)decBHPortfolioValue[i];
                            ...
                            else
                            {
                                ...
                                while (flag)
                                {
                                    ...
                                    {
                                        boolAddRecord = false;
                                        if (objHEEntities.PortfolioValueSet.First(x => x.PortfolioSet.Id.Equals(intID) && x.Date.Equals(dtFillDate)).TREB == null)
                                        {
                                            objHEEntities.PortfolioValueSet.First(x => x.PortfolioSet.Id.Equals(intID) && x.Date.Equals(dtFillDate)).TREB = (float)decBHPortfolioValue[intDateCounter];
                                        }
                                        else
                                        {
                                           // flag = false;
                                            ...
                                            if (dtDiff > 0)
                                            {
                                                ...
                                                while (dtPreviousDate < dtFillDate)
                                                {
                                                    if (!(dtPreviousDate.DayOfWeek.Equals(DayOfWeek.Saturday) || dtPreviousDate.DayOfWeek.Equals(DayOfWeek.Sunday)))
                                                    {
                                                        ...
                                                        {
                                                            if (objHEEntities.PortfolioValueSet.First(x => x.PortfolioSet.Id.Equals(intID) && x.Date.Equals(dtPreviousDate)).TREB == null)
                                                            {
                                                                objHEEntities.PortfolioValueSet.First(x => x.PortfolioSet.Id.Equals(intID) && x.Date.Equals(dtPreviousDate)).TREB = dtPreviousPFValue;
                                                            }
                                                        }
                                                        else
                                                        {
                                                            ...
                                                            objPortfolioValueSet.TREB = dtPreviousPFValue;
                                                            objPortfolioValueSet.PortfolioSet = objHEEntities.PortfolioSet.First(x => x.Id.Equals(intID));
                                                            objHEEntities.AddToPortfolioValueSet(objPortfolioValueSet);
                                                        }
                                                    }
                                    ...
                                    if (boolAddRecord)
                                    {
                                        PortfolioValueSet objPortfolioValueSet = new PortfolioValueSet();
                                        objPortfolioValueSet.Date = dtFillDate;
                                        objPortfolioValueSet.TREB = (float)decBHPortfolioValue[intDateCounter];
                                        objPortfolioValueSet.PortfolioSet = objHEEntities.PortfolioSet.First(x => x.Id.Equals(intID));
                                        objHEEntities.AddToPortfolioValueSet(objPortfolioValueSet);
                                    }
                            ...
                            {
                                //objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).CAGR_RebTrend = (float)Math.Pow((double)(decBHPortfolioValue[decBHPortfolioValue.Count() - 1] / decBHPortfolioValue[217]), (double)(1.00 / ((decBHPortfolioValue.Count() - 1 - 217.00) / 260.00))) - 1.0;
                                objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).CAGR_RebTrend = (float)Math.Pow((double)(decBHPortfolioValue[decBHPortfolioValue.Count() - 1] / decBHPortfolioValue[217]), (double)(1.00 / ((decBHPortfolioValue.Count() - 1 - 217.00) / (double)intTradingDays))) - 1.0;
                                objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).Current_Value_RebTrend = (float)decBHPortfolioValue[decBHPortfolioValue.Count() - 1];
                                objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).Increase_RebTrend = (float)decYearRollChange.Max();
                                objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).DrawDown_RebTrend = (float)decYearRollChange.Min();
                                //objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).Stdev_RebTrend = CommonUtility.STDev(decPortofolioDailyChange.Skip(218).Select(x => (double)x).ToArray()) * (double)Math.Pow(260, 0.5);
                                objHEEntities.PortfolioMethodSet.First(x => x.PortfolioSet.Id.Equals(intID)).Stdev_RebTrend = CommonUtility.STDev(decPortofolioDailyChange.Skip(218).Select(x => (double)x).ToArray()) * (double)Math.Pow(intTradingDays, 0.5);
                            }
                            else
                            {
                                PortfolioMethodSet objMethodSet = new PortfolioMethodSet();
                                //objMethodSet.CAGR_RebTrend = (float)Math.Pow((double)(decBHPortfolioValue[decBHPortfolioValue.Count() - 1] / decBHPortfolioValue[217]), (double)(1.00 / ((decBHPortfolioValue.Count() - 1 - 217.00) / 260.00))) - 1.0;
                                objMethodSet.CAGR_RebTrend = (float)Math.Pow((double)(decBHPortfolioValue[decBHPortfolioValue.Count() - 1] / decBHPortfolioValue[217]), (double)(1.00 / ((decBHPortfolioValue.Count() - 1 - 217.00) / (double)intTradingDays))) - 1.0;
                                objMethodSet.Current_Value_RebTrend = (float)decBHPortfolioValue[decBHPortfolioValue.Count() - 1];
                                objMethodSet.Increase_RebTrend = (float)decYearRollChange.Max();
                                objMethodSet.DrawDown_RebTrend = (float)decYearRollChange.Min();
                                //objMethodSet.Stdev_RebTrend = CommonUtility.STDev(decPortofolioDailyChange.Skip(218).Select(x => (double)x).ToArray()) * (double)Math.Pow(260, 0.5);
                                objMethodSet.Stdev_RebTrend = CommonUtility.STDev(decPortofolioDailyChange.Skip(218).Select(x => (double)x).ToArray()) * (double)Math.Pow(intTradingDays, 0.5);
                            ...
                            foreach (var set in lstAssetAllocation)
                            {
                                if (objHEEntities.PorfolioAllocationInfo.Where(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Count() > 0)
                                {
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Allocation_Method = "TREB";
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Current_Value = (double)set.MAssetValue[set.MAssetValue.Count() - 1];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Current_Weight = (double)set.MAssetValue[set.MAssetValue.Count() - 1] / (double)decBHPortfolioValue[decBHPortfolioValue.Count() - 1];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Starting_Value = (double)set.MAssetValue[217];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Target_Weight = (double)set.MTargetWeight[set.MTargetWeight.Count() - 1];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Share_Price = (double)set.MPrice[217];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).SharePriceTrend = (double)set.MPriceTrend[set.MPriceTrend.Count() - 1];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).CurrentSharePrice = (double)set.MPrice[set.MPrice.Count() - 1];
                                    objHEEntities.PorfolioAllocationInfo.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).CurrentShare = (double)set.MAssetShares[set.MAssetShares.Count() - 1];
                                }
                                else
                                {
                                    PorfolioAllocationInfo objPFAllocationSet = new PorfolioAllocationInfo();
                                    objPFAllocationSet.Allocation_Method = "TREB";
                            ...
                            foreach (var set in lstAssetAllocation)
                            {
                                if (objHEEntities.MostRecentRebalances.Where(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Count() > 0)
                                {
                                    objHEEntities.MostRecentRebalances.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).Allocation_Method = "TREB";
                                    objHEEntities.MostRecentRebalances.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).RebalanceDate = DateTime.FromOADate(set.MDate[intMostRecentRebIndex]);
                                    objHEEntities.MostRecentRebalances.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).OldShare = (double)set.MAssetShares[intMostRecentRebIndex - 1];
                                    objHEEntities.MostRecentRebalances.First(x => x.PortfolioSet.Id.Equals(intID) && x.Allocation_Method.Equals("TREB") && x.AssetSymbolSet.Id.Equals(set.MAssetSymbolId)).NewShare = (double)set.MAssetShares[intMostRecentRebIndex];
                                }
                                else
                                {
                                    MostRecentRebalances objMRR = new MostRecentRebalances();
                                    objMRR.Allocation_Method = "TREB";
                    ...
                    //objPortfolioSet.EndTime = DateTime.Now;
                }
